"""Atlas Scout CLI run command tests."""
...
from .cli_support import _ready_local_model_resolution
...
def test_cli_run_refresh_flag_reaches_runner() -> None:
...
def test_cli_run_verbose_progress_flag_reaches_runner() -> None:
    captured: dict[str, object] = {}
...
@pytest.mark.asyncio
async def test_cli_progress_feed_uses_page_and_entity_events_only(
...
@pytest.mark.asyncio
async def test_cli_progress_feed_hides_internal_progress_by_default(
    tmp_path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from atlas_shared import GapReport
    ...
    async def fake_run_pipeline(**kwargs):
        ...
        on_progress(
            "page_found",
            {"url": "https://example.com/discovered", "depth": 1, "links_found": 10},
        )
        on_progress(
...
@pytest.mark.asyncio
async def test_cli_run_refuses_duplicate_active_direct_run(
    tmp_path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import atlas_scout.cli as cli_module
    import atlas_scout.pipeline as pipeline_module
