...
Use `entries stats` for launch gates and audit checks:
...
exact unique person keys so duplicate run artifacts do not inflate the launch
count.

Passing those gates does not certify currentness. Before syncing or exporting a
