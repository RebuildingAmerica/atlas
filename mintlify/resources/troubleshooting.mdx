...
For a setup-specific path, start with [MCP overview](/mcp/overview). For
permission and scope problems, keep [Scopes and permissions](/mcp/scopes) open
next to this page.
...
Read [Authentication](/api-reference/authentication) for the difference between
...
See [Rate limits](/api-reference/rate-limits) for limits by access pattern.
...
For local searches, compare your request with [Research a place](/api-reference/guides/places)
and [Issue areas & domains](/taxonomy).
