...
<Accordion title="Where does the data come from?">
  ...
  works](/how-it-works) for the full pipeline breakdown.
</Accordion>
...
<Accordion title="Can I correct an entry?">
  Yes. You can [flag an entity](/api/guides/sources-flags) as stale or incorrect
...
<Accordion title="Is Atlas open source?">
  ...
  instance — see the [self-hosting guide](/self-hosting/guide). Contributions
  are welcome.
</Accordion>
...
<Accordion title="Who maintains Atlas?">
  ...
  See [Open source](/resources/open-source) for more.
</Accordion>
...
<Accordion title="How do I get API access?">
  ...
  need an [Atlas Pro or Team plan](/account/pricing) and an API key. Create one
  in your account settings after subscribing.
</Accordion>
...
<Accordion title="How do I connect an AI assistant?">
  ...
  overview](/mcp/overview). MCP requires Atlas Pro or Team.
</Accordion>
...
<Accordion title="What's the Research Pass?">
  A one-time purchase ($9 for 30 days, $4 for 7 days) that gives you Pro-level
  capabilities temporarily — useful for short-term projects without a
  subscription commitment. When it expires, your workspace reverts to Free but
  your saved research remains accessible read-only. See [Plans &
  pricing](/account/pricing).
</Accordion>

<Accordion title="Are there discounts?">
  Yes. Atlas offers 40–50% discounts for verified journalists, grassroots
  nonprofits, and civic tech workers. Apply at
