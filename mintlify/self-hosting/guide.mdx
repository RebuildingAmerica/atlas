...
| `ATLAS_MULTI_USER`                     | `true` for a service with accounts. `false` for a single-user instance — no sign-in, no organizations, no billing. |
| `ATLAS_MANAGED`                    | Leave `false`. It is `true` only on instances the Rebuilding America Project operates, and gates our own billing catalog and staff review surfaces. |
