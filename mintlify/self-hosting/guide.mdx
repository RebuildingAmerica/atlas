...
prices, discount coupons, and webhook endpoints, then write the resulting IDs to
the correct env files.

Local development uses Stripe test mode and a local webhook secret:
...
Use a live secret key, or a live restricted key with permission to manage
products, prices, coupons, Checkout/Billing customers, and webhook endpoints.
Stripe CLI OAuth keys are enough for local and staging checks, but production
bootstrap should use a Dashboard-created live API key.
...
`pnpm bootstrap:stripe:staging` and `pnpm bootstrap:stripe:prod` sync the
`STRIPE_*` billing variables to the linked Vercel project automatically.
