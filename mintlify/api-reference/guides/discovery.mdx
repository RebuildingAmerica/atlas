...
<Note>
  ...
  [authentication](/api-reference/authentication) and [rate
  limits](/api-reference/rate-limits).
</Note>

For exact request and response shapes, use the
[generated API reference](https://api.atlas.rebuildingus.org/docs).
...
| Inspect local research artifacts before syncing | Scout CLI            |
...
source review, or iterative deepening before syncing selected results.
...
>
  Check exact discovery-run request and response schemas.
</Card>
