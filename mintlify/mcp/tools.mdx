...
Ask for the city, issue area, source trail, or research run you need. Your
client chooses the matching Atlas tool.

For guided workflows available through your client's prompt or slash-command
picker, see [Available prompts](/mcp/prompts).

Some tool responses include a `resource_uri`. Pass it to `resources/read` to
open the same research output later. See [Available resources](/mcp/resources).

<Note>
  Tool access depends on your plan, OAuth scopes, and workspace. See [Scopes and
  permissions](/mcp/scopes), [Authentication](/api/authentication), and
  [Workspace integration activity](/workspace/integration-activity) when a tool
  is missing or returns a permission error.
</Note>
...
Because of that, `start_discovery_run` requires an MCP client that supports the [MCP Tasks extension](https://tasks.extensions.modelcontextprotocol.io/specification/draft/tasks): the call returns a task handle immediately, and the client can poll until the run finishes. If your client doesn't support tasks yet, use `list_discovery_runs` or `get_discovery_run` to check on runs you started elsewhere, such as the dashboard.
...
<Card title="Research a city" icon="map" href="/mcp/tutorials/research-city">
  Try the tools in a complete MCP workflow.
</Card>
