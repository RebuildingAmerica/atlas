...
Because of that, `start_discovery_run` requires an MCP client that supports [task-augmented tool calls](https://modelcontextprotocol.io/docs/concepts/tasks): the call returns a task handle immediately, and your assistant polls it until the run finishes rather than blocking. If your client doesn't support tasks yet, use `list_discovery_runs`/`get_discovery_run` to check on runs you started elsewhere (like the dashboard).
