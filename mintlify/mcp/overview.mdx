---
title: "Use Atlas with MCP clients"
description: "Connect Claude, Copilot, Gemini, Codex, and other MCP clients to Atlas."
---
...
(MCP), an open standard for connecting clients to external data and tools.
Claude, GitHub Copilot, Google Gemini, Codex, and other MCP clients can use
Atlas to search civic actors, inspect sources, and start research runs. Ask
questions in natural language and get structured answers backed by Atlas data.
...
When you connect this server, your MCP client can use Atlas search, discovery,
and place-level civic data tools. On first use, the client opens a browser for
Atlas sign-in and access approval.

Atlas also lets MCP clients reopen selected research outputs. Search tools find
the relevant records; returned resource URIs open source-linked briefs, source
trails, and place coverage summaries later. Atlas does not list the entire
entity catalog as MCP resources.

## Available tools

Once connected, Atlas tools include:
...
Atlas resources let MCP clients reopen saved research outputs:
...
resources. Use tools to find records, then use returned `resource_uri` values to
open the same records later.
...
Prompts are read-only in this version. They point clients toward the right
Atlas tools and source-checking steps, but they do not create discovery runs or
