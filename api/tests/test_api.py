...
class TestEntryEndpoints:
    ...
    async def test_create_entry(self, test_client: object) -> None:
        ...
        assert data["full_address"] == "123 Main St, Kansas City, MO 64106"
        assert data["issue_areas"] == ["housing_affordability"]
    ...
    async def test_list_entries_by_state(self, test_client: object, test_db: object) -> None:
        ...
        assert response.status_code == STATUS_OK
        data = response.json()
        assert all(entry["state"] == "MO" for entry in data["data"])
