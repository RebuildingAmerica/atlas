...
from tests.support.hypothesis_strategies import issue_area_slugs
...
class TestSearchTerms:
    ...
    def test_search_terms_count(self) -> None:
        """Test that we have search terms for all 51 issue areas."""
        assert len(ISSUE_SEARCH_TERMS) == EXPECTED_TOTAL_ISSUES


@given(issue_area_slugs())
def test_issue_area_slug_strategy_only_generates_known_issue_areas(issue_area_slug: str) -> None:
    """Hypothesis issue-area strategies should stay aligned with the Atlas taxonomy."""
    issue_area = get_issue_area_by_slug(issue_area_slug)

    assert issue_area is not None
    assert issue_area.slug == issue_area_slug
