...
def _match_type(left: dict[str, Any], right: dict[str, Any]) -> str | None:
    ...
    same_person_type = left.get("entry_type") == "person" and right.get("entry_type") == "person"
    same_organization_type = (
        left.get("entry_type") == "organization" and right.get("entry_type") == "organization"
    )
    ...
    high_similarity_same_city = same_city and similarity >= HIGH_SIMILARITY_THRESHOLD

    result: str | None = None
    if exact_name and same_city and same_type:
        result = "merge"
    elif same_organization_type and exact_name:
        result = "merge" if same_city or len(left.get("name", "").split()) == 1 else "flag"
    elif same_person_type and affiliated_match and exact_name:
        result = "merge"
    elif (
