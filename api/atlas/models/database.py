...
async def _ensure_firehose_signal_columns(conn: Any) -> None:
    """Apply additive Firehose signal migrations before index creation."""
    cursor = await conn.execute("PRAGMA table_info(firehose_signals)")
    rows = await cursor.fetchall()
    if not rows:
        return
    ...
    additive_columns = (
        (
            ...
            "ALTER TABLE firehose_signals ADD COLUMN primary_observation_id TEXT",
        ),
        ("signal_key", "ALTER TABLE firehose_signals ADD COLUMN signal_key TEXT"),
    )
    for column, ddl in additive_columns:
        if column not in existing_columns:
            await conn.execute(ddl)
