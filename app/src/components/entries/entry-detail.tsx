...
export function EntryDetail({
  ...
  if (!entry) {
    return <p className="text-sm text-stone-500">Entry not found.</p>;
  }
      ...
      <Card className="rounded-3xl border-stone-200 bg-white shadow-sm">
        <CardHeader className="space-y-4 border-b-stone-200">
          ...
          <div className="space-y-2">
            <CardTitle className="text-3xl">{entry.name}</CardTitle>
            <p className="text-sm font-medium text-stone-500">{formatLocation(entry)}</p>
            {entry.full_address ? (
              <p className="text-sm text-stone-600">{entry.full_address}</p>
            ) : null}
          </div>
        </CardHeader>
        <CardContent className="space-y-5">
          <p className="text-base leading-7 text-stone-700">{entry.description}</p>
      ...
      <Card className="rounded-3xl border-stone-200 bg-white shadow-sm">
        ...
        <CardContent className="space-y-4">
          {entry.sources?.length ? (
            entry.sources.map((source) => (
              <div key={source.id} className="rounded-2xl border border-stone-200 p-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {source.publication ? (
                    <span className="text-sm font-medium text-stone-700">{source.publication}</span>
                  ) : null}
                  {source.published_date ? (
                    <span className="text-sm text-stone-500">{source.published_date}</span>
                  ) : null}
                </div>
                <a
                  href={source.url}
                  target="_blank"
                  rel="noreferrer"
                  className="mt-3 block text-lg font-semibold text-blue-700 hover:text-blue-800"
                >
            ...
            <p className="text-sm text-stone-500">No linked sources yet.</p>
          )}
        </CardContent>
      </Card>
    </div>
