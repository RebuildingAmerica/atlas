...
export function FilterDisclosure({ count, icon: Icon, items, label }: FilterDisclosureProps) {
  return (
    <Popover className="relative lg:min-w-44 lg:flex-1">
      <PopoverButton className="border-border-strong bg-surface-container-lowest hover:bg-surface-container focus-visible:ring-accent flex w-full cursor-pointer items-center justify-between gap-3 border px-4 py-3 transition-colors outline-none focus-visible:ring-2">
      ...
      <PopoverPanel
        transition
        anchor="bottom start"
        className="border-border bg-surface shadow-soft z-40 mt-2 w-72 origin-top border p-3 transition duration-200 ease-out data-[closed]:opacity-0"
      >
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                className={[
                  "type-label-large inline-flex items-center gap-1.5 border px-2.5 py-1.5 transition-colors",
                  item.active
                    ? "border-accent bg-surface-container-highest text-accent-ink"
                    : "border-border-strong bg-surface-container-low text-ink-soft hover:bg-surface-container-high hover:text-ink-strong",
                ].join(" ")}
              >
