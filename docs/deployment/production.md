...
| ------------------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ATLAS_MULTI_USER`              | Yes                                                                                       | `true` when the instance has accounts, `false` for a single-user instance with no sign-in, organizations, or billing. Everything auth-related is required when this is `true`.                                                          |
| `ATLAS_MANAGED`                 | Yes                                                                                       | `true` only on instances the Rebuilding America Project operates. Gates our own Stripe catalog, staff review surfaces, and cloud-cost posture; a self-hosted instance sets `false` and those surfaces do not exist for it.              |
...
| `ATLAS_AUTH_API_KEY_INTROSPECTION_URL` | Yes when `ATLAS_MULTI_USER` is `true` | Internal URL Atlas uses to validate API keys from the app server. In a Compose deployment, set this to `http://atlas-web:3000/api/auth/internal/api-key`. In a hosted app deployment, set it to the app's public auth route, such as `https://atlas.example.com/api/auth/internal/api-key`. |
...
Atlas workflows explicitly mint GitHub OIDC tokens with
